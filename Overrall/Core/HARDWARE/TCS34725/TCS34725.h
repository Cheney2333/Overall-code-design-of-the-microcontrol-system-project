...
/******************************************************************************/
#define TCS_SDA_IN()  {TCS_SDA_GPIO_Port->MODER &= ~(3 << (TCS_SDA_PIN_NUM * 2));TCS_SDA_GPIO_Port->MODER |= 0 << TCS_SDA_PIN_NUM * 2;}
//{GPIOB->CRH&=0xFFFF0FFF;GPIOB->CRH|=8<<12;}
#define TCS_SDA_OUT() {TCS_SDA_GPIO_Port->MODER &= ~(3 << (TCS_SDA_PIN_NUM * 2));TCS_SDA_GPIO_Port->MODER |= 1 << TCS_SDA_PIN_NUM * 2;}
//{GPIOB->CRH&=0xFFFF0FFF;GPIOB->CRH|=3<<12;}
#define TCS_SDA_READ   HAL_GPIO_ReadPin(TCS_SDA_GPIO_Port, TCS_SDA_Pin)
 
#define TCS_SCL_H     HAL_GPIO_WritePin(TCS_SCL_GPIO_Port, TCS_SCL_Pin, GPIO_PIN_SET) 
#define TCS_SCL_L     HAL_GPIO_WritePin(TCS_SCL_GPIO_Port, TCS_SCL_Pin, GPIO_PIN_RESET)
#define TCS_SDA_H     HAL_GPIO_WritePin(TCS_SDA_GPIO_Port, TCS_SDA_Pin, GPIO_PIN_SET)
#define TCS_SDA_L     HAL_GPIO_WritePin(TCS_SDA_GPIO_Port, TCS_SDA_Pin, GPIO_PIN_RESET)
/******************************************************************************/
#define max3v(v1, v2, v3)   ((v1)<(v2)? ((v2)<(v3)?(v3):(v2)):((v1)<(v3)?(v3):(v1)))
#define min3v(v1, v2, v3)   ((v1)>(v2)? ((v2)>(v3)?(v3):(v2)):((v1)>(v3)?(v3):(v1)))

uint8_t TCS34725_Init(void);
#define TCS_SDA_PIN_NUM 3
